...
app.post('/api/contact', async (req, res) => {
    ...
    if (!emailUser || !emailPass) {
        console.error('Email credentials (EMAIL_USER or EMAIL_PASS) are missing in the server .env file.');
        return res.status(500).json({ 
            error: 'Server email configuration is incomplete. Please set up the credentials in the backend environment file.' 
        });
    }
    ...
    const mailOptions = {
        from: `"${name}" <${process.env.EMAIL_USER}>`, // Send on behalf of user via authenticated SMTP account
        replyTo: email, // Direct replies to the sender's email
